...
async function readFallbackState(fallback: StateStorage, key: string) {
  return Promise.resolve(fallback.getItem(key));
}

async function syncFallbackState(fallback: StateStorage, key: string, value: string) {
  const fallbackValue = await readFallbackState(fallback, key);
  if (fallbackValue !== value) await fallback.setItem(key, value);
}
    ...
    getItem: async (name) => {
      try {
        ...
        if (typeof backendValue === "string") {
          await syncFallbackState(fallback, name, backendValue);
          return backendValue;
        }
      } catch {
    ...
    setItem: async (name, value) => {
      try {
        ...
        if (backendValue === value) {
          await syncFallbackState(fallback, name, value);
          return;
        }
      } catch {
        // Try the write below; the backend may be available for PUT even if GET failed.
      }
